...
const validateRequest = (req, res, next) => {
    const { polygon, featureTypes, forcepolygon, buffersize } = req.body;
    ...
    // Validate buffersize if present (optional parameter)
    if (buffersize !== undefined && (typeof buffersize !== 'number' || buffersize <= 0)) {
      return res.status(400).json({ error: 'buffersize must be a positive number' });
    }
...
app.post('/api/features', validateRequest, async (req, res) => {
    try {
      const { polygon, featureTypes, forcepolygon = false, buffersize } = req.body;
      ...
      // Process and format the response, passing forcepolygon option and buffersize
      const processedFeatures = processOverpassResponse(response.data, forcepolygon, buffersize);
...
function lineToPolygon(coordinates, buffersize) {
    if (coordinates.length < 2) return coordinates;
    
    // Small offset value for creating width
    // Default to 0.00005 (roughly 5-6 meters) if buffersize not provided
    const OFFSET = buffersize !== undefined ? buffersize : 0.00005;
...
function processOverpassResponse(overpassData, forcepolygon = false, buffersize) {
    ...
    const features = overpassData.elements.map(element => {
      ...
      if (element.type === 'way') {
        ...
        // If forcepolygon is true, convert LineString to Polygon
          ...
          geometryType = 'Polygon';
          finalCoordinates = lineToPolygon(coordinates, buffersize);
        }
